    ...
    */
    ...
    // Further infos: https://randomnerdtutorials.com/esp32-pinout-reference-gpios/
        ...
        #define SPISD_CS                    15          // GPIO for chip select (SD)
            ...
            #define SPISD_MOSI              14          // GPIO for master out slave in (SD) => not necessary for single-SPI
            #define SPISD_MISO              12          // GPIO for master in slave ou (SD) => not necessary for single-SPI
            #define SPISD_SCK               13          // GPIO for clock-signal (SD) => not necessary for single-SPI
        #endif
    #endif
    ...
    #define RFID_MOSI                        5          // GPIO for master out slave in (RFID)
    #define RFID_MISO                        4          // GPIO for master in slave out (RFID)
    #define RFID_SCK                         2          // GPIO for clock-signal (RFID)

    #ifdef RFID_READER_TYPE_PN5180
        #define RFID_BUSY                    1          // PN5180 BUSY PIN
        #define RFID_RST                    16          // PN5180 RESET PIN
        #define RFID_IRQ                    99          // PN5180 IRQ PIN (only needed for low power card detection)
    #endif
    // I2S (DAC)
    #define I2S_DOUT                        41          // Digital out (I2S)
    #define I2S_BCLK                        40          // BCLK (I2S)
    #define I2S_LRC                         39          // LRC (I2S)
    ...
    // Rotary encoder
        ...
        #define ROTARYENCODER_DT             9          // Info: Lolin D32 / Lolin D32 pro 35 are using 35 for battery-voltage-monitoring!
    #endif
    ...
    #define NEXT_BUTTON                      6          // Button 0: GPIO to detect next
    #define PREVIOUS_BUTTON                  7          // Button 1: GPIO to detect previous (Important: as of 19.11.2020 changed from 33 to 2; make sure to change in SD-MMC-mode)
    #define PAUSEPLAY_BUTTON                 8          // Button 2: GPIO to detect pause/play
    ...
    // (optional) Neopixel
    #define LED_PIN                         38          // GPIO for Neopixel-signaling

    // (optinal) Headphone-detection
    #ifdef HEADPHONE_ADJUST_ENABLE
        //#define DETECT_HP_ON_HIGH                       // Per default headphones are supposed to be connected if HT_DETECT is LOW. DETECT_HP_ON_HIGH will change this behaviour to HIGH.
        #define HP_DETECT                   42          // GPIO that detects, if there's a plug in the headphone jack or not
    #endif

    // (optional) Monitoring of battery-voltage via ADC
    #ifdef MEASURE_BATTERY_VOLTAGE
        #define VOLTAGE_READ_PIN            10          // GPIO used to monitor battery-voltage. Change to 35 if you're using Lolin D32 or Lolin D32 pro as it's hard-wired there!
    ...
    #ifdef MEASURE_BATTERY_VOLTAGE
        constexpr uint16_t rdiv1 = 442;                              // Rdiv1 of voltage-divider (kOhms) (measure exact value with multimeter!)
        constexpr uint16_t rdiv2 = 160;                              // Rdiv2 of voltage-divider (kOhms) (measure exact value with multimeter!) => used to measure voltage via ADC!
    #endif
